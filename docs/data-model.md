...
| `lastActivityAt`               | `string \| null`                                             | Lifecycle activity clock for inactivity auto-archive. Separate from `updatedAt` and sidebar `statusEnteredAt`. Missing values are written as now on first daemon start after upgrade.         |
| `autoArchiveReason`            | `"merge" \| "inactivity" \| null`                            | Why an automatic archive ran. Support and logging only; not a History badge.                                                                                                                  |
